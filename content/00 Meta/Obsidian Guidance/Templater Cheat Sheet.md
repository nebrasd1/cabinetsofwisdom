...
- When adding templater commands to a template, if you want to place it above the frontmatter so that frontmatter can access the info in it, you need to close out the snippet with ( _ % >) (no spaces)
- The underscore is necessary to prevent new lines to be added that would mess with frontmatter formatting.
- Remember that javascript stuff needs a * in the opening snippet item
## Sites with examples and use cases
- https://shbgm.ca/blog/obsidian/Templater+Snippets
- https://zachyoung.dev/posts/templater-multi-select-suggester/
- https://zachyoung.dev/posts/templater-snippets
## Adding custom option to suggester (only one choice though)
const customOption = "--Add custom option--";
...
## Multiple choices that can be added
let items = ["value", "value2", etc.];
const selectedItems = [];
while (true) {
...
setTimeout(() => {
  ...
  app.fileManager.processFrontMatter(newFile, (frontmatter) => {
    ...
    frontmatter["person"] = frontmatter["person"] || [];
    // Add selected tags to frontmatter
    frontmatter["person"].push(...selectedItems);
  })
}, 300)
}
## Manipulating frontmatter I think?
https://forum.obsidian.md/t/frontmatter-in-templater-seeking-solutions-for-3-use-cases/67670/3
